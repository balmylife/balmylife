...
&emsp;&emsp;一、个人音频文件可以存储到OneDrive，点击分享，复制文件链接，使用[直链](https://link.gimhoy.com/)工具获取。但是会有一个小bug，iPhone端访问会出现音频错误，pc端以及android移动端可以正常访问；如果是使用OneIndex目录映射，可以完美解决此问题。
&emsp;&emsp;二、 网易音乐外链地址获取方法：
